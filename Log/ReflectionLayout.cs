...
namespace Tools.Log
{
    ...
    public class ReflectionPatternConverter : PatternLayoutConverter
    {
        ...
        private object LookupProperty(string property,log4net.Core.LoggingEvent loggingEvent)
        {
            ...
            PropertyInfo propertyInfo = loggingEvent.GetType().GetProperty(property);
            if (propertyInfo != null)
            {
                // propertyValue =propertyInfo.GetValue(loggingEvent.MessageObject, null);
                propertyValue = propertyInfo.GetValue(loggingEvent, null);
            }
            else
            { 
               if(loggingEvent.Properties.Contains(property))
